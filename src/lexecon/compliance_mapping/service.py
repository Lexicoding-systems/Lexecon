...
class ComplianceMappingService:
    ...
    def link_evidence_to_control(
        self, control_id: str, framework: RegulatoryFramework, evidence_artifact_id: str
    ) -> bool:
        """
    ...
    def verify_control(
        self, control_id: str, framework: RegulatoryFramework, notes: Optional[str] = None
    ) -> bool:
        """
    ...
    def get_control_status(
        self, control_id: str, framework: RegulatoryFramework
    ...
    def analyze_gaps(self, framework: RegulatoryFramework) -> List[Dict[str, Any]]:
        """
                ...
                gaps.append(
                    {
                        ...
                        "severity": (
                            "high" if control.status == ControlStatus.NON_COMPLIANT else "medium"
                        ),
                    }
                )
    ...
    def generate_compliance_report(self, framework: RegulatoryFramework) -> ComplianceReport:
        ...
        if compliance_percentage < 80:
            recommendations.append(
                f"Increase compliance coverage from {compliance_percentage:.1f}% to at least 80%"
            )
    ...
    def get_framework_coverage(self, framework: RegulatoryFramework) -> Dict[str, Any]:
        """
        ...
        return {
            ...
            "overall_compliance": (
                (status_counts.get("verified", 0) / total * 100) if total > 0 else 0
            ),
        }
    ...
    def get_primitive_mappings(self, primitive_id: str) -> List[ControlMapping]:
        ...
        """
        return [
            mapping for mapping in self._mappings.values() if mapping.primitive_id == primitive_id
        ]
    ...
    def get_statistics(self) -> Dict[str, Any]:
        ...
        total_controls = sum(len(controls) for controls in self._control_registry.values())
        ...
        return {
            ...
            "overall_verification_rate": (
                (verified_controls / total_controls * 100) if total_controls > 0 else 0
            ),
        }
