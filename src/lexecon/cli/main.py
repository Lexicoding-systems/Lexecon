...
def decide(json_file: Optional[str], actor: Optional[str], action: Optional[str],
           tool: Optional[str], intent: Optional[str]):
    ...
    if json_file:
        ...
        click.echo("Error: Provide either --json or all of (--actor, --action, --tool, --intent)",
                   err=True)
        sys.exit(1)
...
@cli.command()
@click.option("--policy-file", required=True, type=click.Path(exists=True),
...
@cli.command()
@click.option("--ledger-file", required=True, type=click.Path(exists=True),
              help="Ledger JSON file")
